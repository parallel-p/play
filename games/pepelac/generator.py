...
class Generator:
    def generate_players(self, field, players_count):
        ...
        for player in range(players_count):
            player_x = random.randint(0, self._field_size - 1)
            player_y = random.randint(0, self._field_size - 1)
    ...
    def generate_bullets(self, field, bullets_count):
        ...
        for bullet in range(bullets_count):
            bullet_x = random.randint(0, self._field_size - 1)
            bullet_y = random.randint(0, self._field_size - 1)
    ...
    def generate_start_positions(self, game_signature, players):
        ...
        self._field_size = random.randint(40, 50)
        self.bullets_count = self.players_count * 5
        for game in range(self.players_count):
            field = [[0 for i in range(self._field_size)]
            ...
            yield JuryState(self._field_size, field, self.bullets, self.time,
                players, [])
